...
function is_possible(total::Int, arguments::Vector{Int})
    ...
    for i in 0:(length(available_operations)^(length(arguments)-1)-1)
        arguments_copy = copy(arguments)
        choices = digits(Int, i; base = length(available_operations), pad = (length(arguments)-1))
        ...
        for i in eachindex(operations)
            ...
            elseif operations[i] == '*'
                ...
                arguments_copy[1] = parse(Int, join([arguments_copy[1], arguments_copy[i+1]]))
            end
        end
